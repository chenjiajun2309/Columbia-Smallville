# -*- coding: utf-8 -*-
"""Entry for Columbia Smallville simulation."""
...
# ---------- anchor paths (important!) ----------
HERE = os.path.dirname(os.path.abspath(__file__))                   # .../generative_agents
...
# use package-qualified imports so both `python -m` 和直接 python 都能跑
from generative_agents.modules.game import create_game, get_game
...
def get_config_from_log(checkpoints_folder):
    ...
    json_files = [os.path.join(checkpoints_folder, fn)
                  for fn in files if fn.endswith(".json") and fn != "conversation.json"]
    if len(json_files) < 1:
        return None
    ...
    assets_root = os.path.join("assets", "Columbia")
...
def get_config(start_time="20240213-09:30", stride=15, agents=None):
    # 读取 generative_agents/data/config.json
    cfg_path = os.path.join(HERE, "data", "config.json")
    with open(cfg_path, "r", encoding="utf-8") as f:
        json_data = json.load(f)
        agent_config = json_data["agent"]

    assets_root = os.path.join("assets", "Columbia")  # 相对 static_root 的路径
    config = {
        "stride": stride,
...
parser = argparse.ArgumentParser(description="Console for Columbia")
parser.add_argument("--name", type=str, default="", help="The simulation name")
parser.add_argument("--start", type=str, default="20240213-09:30", help="Start time, e.g. 20240213-09:30")
parser.add_argument("--resume", action="store_true", help="Resume from checkpoints")
parser.add_argument("--step", type=int, default=10, help="Number of simulate steps")
parser.add_argument("--stride", type=int, default=10, help="Minutes per step")
parser.add_argument("--verbose", type=str, default="debug", help="Verbose level")
parser.add_argument("--log", type=str, default="", help="Log file name under checkpoints")
args = parser.parse_args()


if __name__ == "__main__":
    # 把路径锚定在包内 results/checkpoints
    checkpoints_path = os.path.join(HERE, "results", "checkpoints")
    ...
    if resume:
        while not os.path.exists(f"{checkpoints_path}/{name}"):
            name = input(f"'{name}' doesn't exist, please re-enter the simulation name: ")
    else:
    ...
    # static_root 指向包内 frontend/static
    static_root = os.path.join(HERE, "frontend", "static")
